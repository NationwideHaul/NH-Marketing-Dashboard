-- Bounced is now a count (number), not a rate. Final email metric shape:
-- delivered (count), open_rate (%), click_rate (%), replied (count),
-- bounced (count). Table holds no real data yet, so recreate it.
